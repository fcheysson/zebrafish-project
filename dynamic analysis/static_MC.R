...
for (fish in names) {
    ...
    plist.MC = solapply(cliches, function(image) {
        cells.MC %>% filter(cliche == image) %>% 
        {ppp(x = .$dim1, y = .$dim2, window = cells.owin, marks = as.factor(.$type))} %>% 
            rescale(s=cells.diameter, unitname="cell diameter")
    })
    ...
    Leach = lapply(plist.MC, Lest, r=seq(0, 6, by=0.1), ratio=TRUE, correction="best")
    ...
    ggplot(EL.simultaneous, aes(x=r)) +
        ...
        ylab(expression(italic(L["MC, MC"](r)))) +
...
pdf(paste0("pval_MC-MC.pdf"), height=2, width=3)
